 Shader "Hidden/tiltShift" {
	...
	half4 fragCocBgAfterFg (v2f i) : COLOR 
	{
		...
		return max( EncodeFloatRGBA (coc), tex2D (_Coc, i.uv.xy));
	}
	...
	{
		half4 color = tex2D (_MainTex, i.uv.xy);
		half4 blurred =  (half4(0,1,0,1) + tex2D (_Blurred, i.uv.xy)) * 0.5;
		half coc = DecodeFloatRGBA (tex2D (_Coc, i.uv.xy));
	...
	{
		half4 color = tex2D (_MainTex, i.uv.xy);
		half4 oldCoc = tex2D (_Coc, i.uv.xy);
		
		return max (color, oldCoc);
	}	
